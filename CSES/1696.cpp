...
void addedge(long long u, long long v, long long c){
    ...
    el[nbedge+1][0] = v; el[nbedge+1][1] = u; el[nbedge+1][2] = 0;
...
long long dfs(long long node, long long pushed){
    ...
    if(node == sink){
        if(pushed>0){ans.push_back(node);}
        return pushed;
    }
...
long long maxflow(bool print){
    ...
    while(bfs()){
        ans.clear();
        ret += dfs(source, 1e18);
        if(print){cout << ans.size() << endl;}
        for(int i=ans.size()-1; print && i>=0; i--){
            cout << ans[i] << " ";
        }
        if(print){cout << endl;}
        //cout << ret << endl;
